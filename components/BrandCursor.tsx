...
import { playEdgeClick } from "@/lib/brand/edge-sound";
...
function isPrimaryCta(el: EventTarget | null): boolean {
  if (!(el instanceof Element)) return false;
...
export default function BrandCursor() {
  ...
  useEffect(() => {
    ...
    const paint = () => {
      ...
      commitMix.current += (0 - commitMix.current) * 0.12;
      const mix = actionMix.current;
      const commit = commitMix.current;
      const headScale = 1 + mix * 0.72 + commit * 0.55;
      const ringScale = 0.55 + mix * 0.95 + commit * 0.35;
      ...
      if (ring) {
        const p = pts[0];
        ring.style.opacity = String(mix * 0.9 + commit * 0.2);
        ring.style.transform = `translate3d(${p.x}px, ${p.y}px, 0) translate(-18px, -18px) scale(${ringScale})`;
      }

      if (headPath) {
        headPath.setAttribute("stroke", mix > 0.35 || commit > 0.2 ? "#ffffff" : "none");
        headPath.setAttribute("stroke-width", mix > 0.35 || commit > 0.2 ? "1.1" : "0");
        headPath.setAttribute("fill", mix > 0.5 || commit > 0.2 ? "#FF8A2B" : "#FF6F00");
      }
    ...
    const onClick = (e: MouseEvent) => {
      if (!isPrimaryCta(e.target)) return;
      commitMix.current = 1;
      playEdgeClick();
    };
